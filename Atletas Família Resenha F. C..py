...
with st.form("form_carteirinha"):
    ...
    with col1:
        nome = st.text_input("Nome Completo", placeholder="Ex: Ricardo Stevanatto Calvoso")
        apelido = st.text_input("Apelido na Resenha *", placeholder="Ex: Calvoso")
        nascimento = st.text_input("Data de Nascimento", placeholder="Ex: 03/09/1982")
        cidade_uf = st.text_input("Cidade / UF", value="São Paulo / SP")
        
    with col2:
        camisa = st.text_input("Número da Camisa *", placeholder="Ex: 3")
        inicio = st.text_input("Temporada de Início", value="2026")
        pe = st.selectbox("Pé Dominante", ["Ambidestro", "Diestro", "Canhoto"])
...
if submitted:
    if not apelido or not camisa:
        ...
        # Dimensões do Cartão (Proporção HD - 1012x638 px)
        W, H = 1012, 638
        card = Image.new("RGB", (W, H), (20, 24, 33)) # Fundo Azul Escuro
        ...
        # Textos do Cabeçalho
        draw.text((30, 25), "FAMÍLIA RESENHA F.C.", fill=(250, 204, 21), font=font_large)
        draw.text((30, 65), "CARTEIRINHA VIRTUAL DE SÓCIO-ATLETA", fill=(203, 213, 225), font=font_large)
        ...
        if foto_file:
            ...
            # Desenha quadro placeholder se não houver foto
        ...
        # Seção do Teor Alcoólico (Barra de Progresso)
        ...
        if bar_w > 0:
            # Cor varia conforme o nível de teor alcoólico
        ...
        st.success("Carteirinha gerada com sucesso! Veja o resultado abaixo:")
